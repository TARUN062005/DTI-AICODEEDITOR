...
import { Send, Cpu, X, Copy, Loader2 } from "lucide-react"
...
export function AICommandCenter({ isOpen, onClose }: AICommandCenterProps) {
  ...
  if (!isOpen) {
    return (
      <Button
        className="fixed bottom-6 right-6 z-50 h-12 w-12 rounded-full shadow-lg bg-emerald-600 hover:bg-emerald-700"
        onClick={onClose}
      >
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-80 rounded-lg border border-slate-800 bg-slate-900 shadow-xl sm:w-96">
      <div className="flex items-center justify-between border-b border-slate-800 p-3">
        <div className="flex items-center gap-2">
          <div className="rounded-full bg-emerald-600/20 p-1">
            <Cpu className="h-4 w-4 text-emerald-500" />
          </div>
          <span className="font-medium text-slate-200">AICODE Assistant</span>
        </div>
